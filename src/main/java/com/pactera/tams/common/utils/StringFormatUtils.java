...
public class StringFormatUtils {
	...
	public static String format(String format, Object param, PostHandleIface postHandler) {
		...
		// 存储结果
		Map<String, Object> varNameToValueMap = new HashMap<String, Object>(16);
		// 提取所有的占位符
	...
	private static Map<String, String> extractVariable(Set<String> targetVarReplacementSet) {
		Map<String, String> varNameToVarReplacementMap = new HashMap<String, String>(16);
	...
	private static String extractVariable(String varRepalcement) throws IllegalArgumentException {
		...
		if (varRepalcement == null) {
			// variableString = null;
		} else if (varRepalcement.length() <= length) {
			varName = "";
		} else {
	...
	private static void validateVariableName(String varName) throws IllegalArgumentException {
		String espStr = ".";
		if (isStringEmpty(varName) || varName.startsWith(espStr) || varName.endsWith(espStr)
				|| varName.matches("[.][ ]*[.]")) {
	...
	private static void extractValues(final Object targerObject, final String variablePrefix,
		...
		Map<String, TreeSet<String>> locatedVarToSuccessorVarSetMap = new HashMap<String, TreeSet<String>>(16);
	...
	public static class NullAsStringPostHandler implements PostHandleIface {
	   ...
	   * @return
	   */
	  public static List getListByStrFormat(String s,String charac) {
		  List list = new ArrayList();
		  String[] sArray = s.split(charac);
	   ...
	   * @return
	   */
	  public static String getConditionStr(List list,String str) {
	  	StringBuffer condition = new StringBuffer();
	  	condition.append("(");
	  	for(int i=0;i<list.size();i++) {
	  		if(i + 1 == list.size() ) {
	  			condition.append("instr("+str+",',"+list.get(i)+",') > 0 ");
	  		}else {
	  			condition.append("instr("+str+",',"+list.get(i)+",') > 0 or ");
	  		}
	  	}
	  	condition.append(")");
	  ...
	  /**
	   * 结果信息
	   * @return  retCode 1表示成功   0 表示失败
	   */
	  public static ModelMap getResultMessage() {
